    ...
    def display(self, class_obj):
        for i in range(0, self.square_count):
            ...
            if (i == self.active_index):
                ...
                #cv2.putText(overlay, self.a[i], (int((x + self.rect   _width / 2 - 40)), int((y + self.rect_width / 2 + 40))),cv2.FONT_HERSHEY_PLAIN, 7, (255, 255, 255), 10)
                print("x:", x)
                print("y:", y)
                overlay[y:y+150, x:x+150] = self.img //2
                #class_obj.image = cv2.addWeighted(overlay, alpha, class_obj.image, 1 - alpha, 1)
            else:
                #cv2.rectangle(overlay, (x, y), (x + self.rect_width, y + self.rect_width), (int(50 * i), 0, 0), -1)
                #cv2.putText(overlay, self.a[i], (int((x + self.rect_width / 2 - 40)), int((y + self.rect_width / 2 + 40))),cv2.FONT_HERSHEY_PLAIN, 7, (255, 255, 255), 10)
                print("x:", x)
                print("y:", y)
                overlay[y:y+150, x:x+150] = self.img
...
class HandControlVolume:
    ...
    def recognize(self):
        ...
        # 视频分辨率
        resize_w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)) * 4
        ...
        # 创建多个方块
        for i in range(0, 5):
            position = random.randint(20, resize_w - 150)
            # position = random.choice([20, 220, ])
            # squareManager.create(200 * i + 20, 200, 0.6)
            squareManager.create(position, 200, 0.6)
                ...
                if results.multi_hand_landmarks:
                    ...
                    for hand_landmarks in results.multi_hand_landmarks:
                        ...
                        if landmark_list:
                            ...
                            elif (line_len < 80) and (squareManager.checkOverlay(between_finger_tip[0],
